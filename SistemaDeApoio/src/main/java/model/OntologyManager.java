...
public class OntologyManager {
        ...
        for (OWLClass c : localOntology.getClassesInSignature()) {
            ...
            for (OWLNamedIndividual node : instances.getFlattened()) {
                if (node.toString().indexOf(documentIRI + "#" + instance1) != -1) {
                    System.out.println(node + "\t" + c.toStringID());
                    if (c.getSuperClasses(localOntology).toString().contains("tipos_de_investimento")) {
                        System.out.println("super " + c.getSuperClasses(localOntology));
                    }
    ...
    public List<String> showClassAfterReasoningOfInstance(String instance1) {
        //StringBuilder sb = new StringBuilder();
        List<String> perfil_investimento = new ArrayList<>();
        String perfil = "", investimento = "";
        ...
        for (OWLClass c : localOntology.getClassesInSignature()) {
            ...
            for (OWLNamedIndividual node : instances.getFlattened()) {
                if (node.toString().indexOf(documentIRI + "#" + instance1) != -1) {
                    if (c.getSuperClasses(localOntology).toString().contains("tipos_de_investimento")) {
                        System.out.println("tipos de investimento " + c.getSuperClasses(localOntology));
                        System.out.println("tipos de investimento " + c);

                        investimento += c.toString();

                    }
                    //gambiarra master, time is running out
                    if (c.getSuperClasses(localOntology).toString().contains("perfil")
                            && (c.toString().contains("agressivo")
                            || c.toString().contains("conservador")) || c.toString().contains("moderado")) {
                        System.out.println("aspectos_do_investimento " + c.getSuperClasses(localOntology));
                        System.out.println("aspectos_do_investimento " + c);
                        perfil += c.toString();
                    }
        ...
        perfil_investimento.add(0, investimento);
        perfil_investimento.add(1, perfil);

        return perfil_investimento;
    }
